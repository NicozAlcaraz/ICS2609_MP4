...
<%
    ...
    // if session does not exist yet then bring user to the login page
    if (session.getAttribute("user") == null) {
        response.sendRedirect(request.getContextPath() + "/error_session.jsp");
        return;
    }

    // get the email from the session and store it in a variable
    ...
    <body>
        ...
        <main>
            <div class="box">
                ...
                <p>Your role is: <span><%= role%></p>
                <form action="PDFServlet" method="post" target="_blank">
                    <button type="submit" value="submit">PDF Report</button>
                </form>

                <form action="${pageContext.request.contextPath}/controller/LogoutServlet" method="post" id="logoutForm">
                    <input type="submit" value="Log Out" onclick="document.getElementById('logoutForm').submit();"/>
                </form>
            </div>
        </main>
        <footer>
